...
class Data_Handler:
    ...
    def plot_grid(X, y, type="MRI", predictions=None):
        ...
        if type not in ["MRI", "XRAY", "PREDICT"]:
            print("Invalid type specified. Setting type to 'GRID' for generic plotting.")
            type = "GRID"

        if type == "PREDICT" and predictions is not None:
            predictions = predictions[:num_rows]  # Ensure predictions match the number of samples
        ...
        # Set up a plot with num_rows and num_cols
        fig, axes = plt.subplots(num_rows, num_cols, figsize=(num_cols * 3, num_rows * 3))

        # Handle edge case where num_rows is 1 (axes becomes 1D instead of 2D)
        if num_rows == 1:
            axes = [axes]
        ...
        for i in range(num_rows):
            ...
            if type == "PREDICT" and predictions is not None:
                # Use [:, :, 0] because predictions shape is (H, W, 1)
...
def main():
    ...
    Data_Handler.plot_grid(X=X_mri, y=y_mri, type="MRI")
    Data_Handler.plot_grid(X=X_xray, y=y_xray, type="XRAY")
